    ...
    test "statistics with correct credentials needs be return a valid statistic for a player" do
      ...
      assert WowMetrics.statistics(@token, @player) == {
               ...
               %{
                 "corruption" => %{
                   "corruption" => 125.0,
                   "corruption_resistance" => 81.0,
                   "effective_corruption" => 44.0
                 }
               }
             }
    end
    ...
    test "players_statistics with correct credentials needs be return a valid list with WowMetrics.Player" do
      expect(Mock, :get!, fn _, _ ->
        %HTTPoison.Response{
          status_code: 200,
          body:
            "{\"character\":{\"name\":\"Nikolei\"},\"corruption\":{\"corruption\":123.0,\"corruption_resistance\":82.0,\"effective_corruption\":123.0}}"
        }
      end)
      ...
      expect(Mock, :get!, fn _, _ ->
        %HTTPoison.Response{
          status_code: 200,
          body:
            "{\"character\":{\"name\":\"Junkratxd\"},\"corruption\":{\"corruption\":124.0,\"corruption_resistance\":87.0,\"effective_corruption\":456.0}}"
        }
      end)
      ...
      expect(Mock, :get!, fn _, _ ->
        %HTTPoison.Response{
          status_code: 200,
          body:
            "{\"character\":{\"name\":\"Faölin\"},\"corruption\":{\"corruption\":127.0,\"corruption_resistance\":85.0,\"effective_corruption\":789.0}}"
        }
      end)
